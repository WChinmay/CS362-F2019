...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case feast:
        ...
        while( x == 1) {//Buy one card
            if (supplyCount(choice1, state) <= 0) {
                ...
                discardCard(handPos, currentPlayer, state, 1);//discard card
                if (DEBUG) {
    ...
    case mine:
        ...
        if ( (getCost(state->hand[currentPlayer][choice1]) + 3) < getCost(choice2) )   // Bug #1 FIX
        {
            return -1;
        }
        ...
        for (i = 0; i < state->handCount[currentPlayer]; i++)
        {
            if (state->hand[currentPlayer][i] == j)
            {
                ...
                discardCard(i, currentPlayer, state, 1);   // Bug #2 FIX
                break;
            }
        }
    ...
    case remodel:
        ...
        if ( (getCost(state->hand[currentPlayer][choice1]) + 2) < getCost(choice2) )    // Bug #3 FIX
        {
            return -1;
        }
    ...
    case ambassador:
        ...
        for (i = 0; i < state->handCount[currentPlayer]; i++)
        {
            if (i != handPos && state->hand[currentPlayer][i] == state->hand[currentPlayer][choice1] && state->hand[currentPlayer][i] != choice1)
            {
                j++;
            }
        }
