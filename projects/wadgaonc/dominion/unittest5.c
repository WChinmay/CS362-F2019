...
int main()
{
    ...
    printf("Begin Testing handleMine():\n");
    ...
    // handleMine(struct gameState *state, int choice1, int choice2, int currentPlayer, int handPos)
    // call the refactored function
    // This unit test checks if the card indicated by supplyPos is added to the discard pile
    // of the current player
    printf("supplyPos is %d\n", test1);
    int r1 = handleMine(&G, choice1, test1, 0, 0);
    ...
    int r2 = handleMine(&G, choice1, test1, 0, 0);
    test2 = G.handCount[0];
